...
#
$ErrorActionPreference = 'Continue'   # don't let a single native stderr line abort
...
function Is-StoreShim($src) { return ($src -like "*\WindowsApps\*") }  # MS Store App-Execution-Alias
...
function Find-RealPython {
  ...
  foreach ($n in 'python','python3') {
    ...
    if ($c -and -not (Is-StoreShim $c.Source)) { $cands += $c.Source }
  }
  $cands += @(
...
function Find-Git {
  ...
  if ($c -and -not (Is-StoreShim $c.Source)) { return $c.Source }
  foreach ($p in @("$env:ProgramFiles\Git\cmd\git.exe","$env:ProgramFiles\Git\bin\git.exe","${env:ProgramFiles(x86)}\Git\cmd\git.exe")) {
...
# --- 0. Toolchain: real git + real Python 3.12 (ignore Store shims) ----------
$git = Find-Git
if (-not $git) { Winget-Install Git.Git 'Git'; $git = Find-Git }
Rec 'git available' ([bool]$git) "$git"

$py = Find-RealPython
if (-not $py) { Winget-Install Python.Python.3.12 'Python 3.12'; $py = Find-RealPython }
if ($py) { $pyver = & $py --version 2>&1 } else { $pyver = 'NONE' }
Rec 'Python 3.12 available' ([bool]$py -and "$pyver" -match '3\.1[0-2]') "$pyver at $py"
if (-not $py -or -not $git) { Write-Host "FATAL: missing toolchain; aborting." -ForegroundColor Red; Stop-Transcript | Out-Null; return }
...
# --- 7. .gitattributes keeps shell scripts LF -------------------------------
$ga = Get-Content (Join-Path $Repo '.gitattributes') -Raw -ErrorAction SilentlyContinue
...
Write-Host "Installing requirements (downloads a few hundred MB)..." -ForegroundColor Yellow
& $vpy -m pip install --upgrade pip --quiet
& $vpy -m pip install -r requirements.txt 2>&1 | Select-Object -Last 3
$pipOk = ($LASTEXITCODE -eq 0) -and (Test-Path $vpy)
Rec 'pip install -r requirements.txt' $pipOk 'deps installed into venv'

# --- 2. setup/login (data dirs, DB, admin) ----------------------------------
$env:HF_HUB_DISABLE_SYMLINKS = '1'
& $vpy setup.py 2>&1 | Out-String | Out-Null
